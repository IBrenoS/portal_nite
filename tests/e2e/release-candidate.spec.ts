...
test("fluxo principal home -> projeto -> retorno funciona", async ({
  page,
}) => {
  await page.goto("/");

  await page
    .getByRole("link", { name: "Ver projeto: Software aplicado" })
    .click();

  await expect(page).toHaveURL(/\/projetos\/software-aplicado$/);
  await expect(
    page.getByRole("heading", { level: 1, name: "Software aplicado" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: "Projetos em movimento" }),
  ).toBeVisible();
});

test("release candidate mantem animacao estavel em reload, idle e navegacao repetida", async ({
  page,
}) => {
  ...
  expect(
    Math.abs(logoAfterIntro!.width - logoAfterIdle!.width),
  ).toBeLessThanOrEqual(1);
  expect(
    Math.abs(logoAfterIntro!.height - logoAfterIdle!.height),
  ).toBeLessThanOrEqual(1);
  ...
  const logoOutsidePanel = await page
    .locator("#logo-final")
    .evaluate((logo) => !logo.closest(".brand-panel"));
...
for (const viewport of [
  ...
  test(`release candidate nao quebra layout em ${viewport.name}`, async ({
    page,
  }) => {
