...
- [x] Fechar inconsistencia de navegacao MVP para `/contato`. Evidencia: microauditoria de rotas confirmou `/contato` acessivel pelo grupo Contato do Header e pelo Footer, Header sem CTA global isolado, `/contato` renderizando com H1 unico e sem formulario/canal ficticio, rotas MVP respondendo em browser local e build listando `/`, `/projetos`, `/projetos/[slug]`, `/oportunidades`, `/atualizacoes` e `/contato`.
