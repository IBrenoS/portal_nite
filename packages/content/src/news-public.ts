...
import * as cmsSchema from "./db/schema";
import { publishedArticles, type PublishedArticleRow } from "./db/schema";
import {
...
type NewsDatabase = Pick<
  PgDatabase<PgQueryResultHKT, typeof cmsSchema>,
  "select"
>;
...
function buildPublicMediaUrl(baseUrl: string, objectKey: string): string {
  const base = new URL(baseUrl.endsWith("/") ? baseUrl : `${baseUrl}/`);

  if (base.protocol !== "https:" && base.protocol !== "http:") {
    throw new Error("A URL publica de midia deve usar HTTP ou HTTPS.");
  }
  ...
  if (
    ...
    throw new Error("A chave publica de midia e invalida.");
  }
...
export function createDrizzleNewsPublicDataSource(
  database: NewsDatabase,
  options: { mediaBaseUrl: string },
): NewsPublicDataSource {
  return {
    async listPublishedArticles() {
      const rows = await database.select().from(publishedArticles);

      return rows.map((row) => mapPublishedArticle(row, options.mediaBaseUrl));
    },
  };
}

export function createNeonNewsPublicDataSource(options: {
  databaseUrl: string;
  mediaBaseUrl: string;
}): NewsPublicDataSource {
  const client = neon(options.databaseUrl);
  const database = createNeonDatabase(client, { schema: cmsSchema });

  return createDrizzleNewsPublicDataSource(database, options);
}
