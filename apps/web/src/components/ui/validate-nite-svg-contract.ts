...
export function validateNiteSvgContract(
  root: SVGSVGElement | HTMLElement,
): NiteSvgContractCounts {
  ...
  if (missing.length > 0) {
    throw new Error(
      `[NITE SVG] Missing required selectors: ${missing.join(", ")}`,
    );
  }

  if (counts.overlays !== 1) {
    throw new Error(
      `[NITE SVG] Expected exactly one #energy-overlay. Found: ${counts.overlays}`,
    );
  }

  if (counts.mainRise < 3) {
    throw new Error(
      "[NITE SVG] Expected at least 3 bulb-to-brain energy paths.",
    );
  }
