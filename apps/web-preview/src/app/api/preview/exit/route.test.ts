...
describe("POST /api/preview/exit", () => {
  ...
  it("desabilita Draft Mode, remove a sessão e redireciona com 303", async () => {
    const response = await POST(
      new Request("https://nite.test/api/preview/exit", { method: "POST" }),
    );
  ...
  it.each(["https://externo.test", "/%5Cevil.test/path", "//evil.test/path"])(
    "rejeita retorno externo: %s",
    async (returnTo) => {
      const response = await POST(
        new Request(
          `https://nite.test/api/preview/exit?returnTo=${encodeURIComponent(returnTo)}`,
          { method: "POST" },
        ),
      );
      expect(response.headers.get("location")).toBe(
