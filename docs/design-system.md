...
- **Estrutura:** hero cinematográfico em tela cheia, destaque `lead` sobreposto à base da cena, grade `standard`, agenda `compact` e página de leitura dedicada. Os filtros editoriais permanecem compartilháveis pela URL, sem barra visual dentro do hero.
...
- **Blocos editoriais:** cada matéria inteira é um link semântico plano, com imagem arredondada, categoria, título, resumo quando aplicável e metadata diretamente sobre o fundo do tema. Não há borda, fundo, sombra, escala, pulso ou transição geométrica; hover e foco alteram apenas a cor do título, além do ring de foco. O destaque usa `h2`; blocos dentro de seções usam `h3`. `lead` organiza imagem e texto em duas colunas no desktop; `standard` e `compact` empilham imagem e texto, com `compact` omitindo o resumo.
...
- **Radar de Sinais:** canvas 2D estritamente decorativo com dez órbitas circulares em todos os breakpoints. O centro lógico fica em `(viewportWidth / 2, viewportHeight × 1.25)`; os raios partem de `150px`, recebem incremento inicial de `127.5px` e esse incremento cresce `1.1×` após cada órbita. O canvas mantém dimensões lógicas da viewport e é escalado pelo stage, revelando cinco ápices principais antes do destaque em `1920×958` e recortando as órbitas externas como na referência.
- **Halo do Radar:** reutiliza `/images/projetos/projects-hero-light.png` (`963×1019`) com largura-base de `868px`, `max-width: 100%` e proporção intrínseca preservada. Fica a `80px` da base, com `translateY(33.333%)` e rotação de `235deg`; no mobile, reduz até a largura disponível do stage. No dark, o canvas usa preto puro e um campo azul `#2563eb` de `70vw` (`100%` no mobile), blend `color` e máscara radial de `20%` a `90%`; a composição recebe ainda `/images/projetos/news-hero-noise.png` em `cover`, centralizado e com `30%` de opacidade para reproduzir a granulação da referência. No light, o tingimento azul a `18%` usa diretamente o alpha do PNG como máscara, sem ruído nem retângulo cromático.
- **Geometria editorial:** stage de `90vh` com máximo de `42rem` no mobile e `calc(100vh - 3.75rem)` no desktop, sem `min-height`. O título começa a aproximadamente `128px` no mobile; no desktop, o wrapper usa `top: calc(50% - 16.6875rem)`. O destaque ocupa `90%`, até `72rem`, e sobrepõe o stage em `96px` no mobile e `140px` a partir de `md`.
- **Motion:** cada pulso alterna suavemente entre velocidades independentes de `0.65×` a `1.45×`, sem papel fixo por órbita. A poeira mantém a quantidade de pontos, mas estrelas desaparecem, são reposicionadas somente quando invisíveis e reaparecem gradualmente. O renderer limita a 30 FPS e pausa fora da viewport ou com a aba oculta; `prefers-reduced-motion` mantém um frame estático e determinístico.
...
O painel administrativo mantém tokens, tipografia, primitives e padrões de acessibilidade próprios no repositório CMS. Ele privilegia densidade operacional e clareza de estado e não depende do design system, do hero, das cenas cinematográficas ou dos cards editoriais da vitrine pública.

No Portal, tokens e primitives estáveis ficam em `@nite/ui`. O CMS mantém sua implementação independente em `@nite/cms-ui`; não existe package de UI compartilhado entre repositórios. Elementos específicos do editor, shell e upload permanecem em `apps/admin` no repositório CMS.
...
- **Editor:** a área de conteúdo mantém largura de leitura confortável e toolbar acessível por teclado. O preview usa o renderer local do CMS e valida o mesmo shape versionado que a API pública expõe; paridade visual não depende de imports do Portal.
